...
def ppo_train(env, agent, epochs=2, steps_per_epoch=2048, gamma=0.99, lam=0.95,
    ...
    training_log = {
        ...
        "value_loss": [],
        "total_loss": [],
        "episodes": [],  # to record episode total rewards, etc.
        "step_sars": []  # option: record s,a,r,done for each step
    }
    ...
    for epoch in range(epochs):
        ...
        for t in range(steps_per_epoch):
            ...
            if done:
                ...
                # Log episode reward for this epoch
        ...
        dataset = torch.utils.data.TensorDataset(obs_tensor, actions_tensor, log_probs_tensor, returns_tensor,
                                                 adv_tensor)
        ...
        training_log["policy_loss"].append(epoch_policy_loss / update_count)
        training_log["value_loss"].append(epoch_value_loss / update_count)
        training_log["total_loss"].append(epoch_total_loss / update_count)

        print(f"[epoch {epoch + 1}] mean reward: {np.mean(ep_rewards) if ep_rewards else 'n/a'}")

        # Save log after each epoch (optional)
